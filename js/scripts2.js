...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const priceRange = $('#price_range').ionRangeSlider({
        ...
        onChange: data => {
            // При изменении слайдера, обновляем поля ввода
            $('.price_range input.from').val(data.from + priceRangePostfix);
            $('.price_range input.to').val(data.to + priceRangePostfix);
            
            // Здесь же можно отправить форму, если хотите "мгновенную" фильтрацию
            // Но обычно для range-слайдера предпочтительнее делать это на 'onFinish'
            // или после нажатия кнопки "Применить фильтры".
        },
        onUpdate: data => {
            // Также обновляем поля ввода при программном обновлении слайдера
            $('.price_range input.from').val(data.from + priceRangePostfix);
            $('.price_range input.to').val(data.to + priceRangePostfix);
        },
        /*onFinish: function () {
            // При отпускании ползунка, отправляем форму
            $('.form_filter').submit();
        }*/
    }).data('ionRangeSlider');

    // Обработчик для ручного ввода в поля
    $('.price_range .input').on('change', function () { // Используем 'change' вместо 'keyup' для отправки формы
        // Обновляем слайдер значениями из полей ввода
        priceRange.update({
